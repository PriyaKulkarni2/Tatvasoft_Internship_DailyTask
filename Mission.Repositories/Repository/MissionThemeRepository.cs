...
namespace Mission.Repositories.Repository
{
    public class MissionThemeRepository(MissionDbContext dbContext) : IMissionThemeRepository
    {
        ...
        public async Task AddMissionThemeAsync(UpsertMissionThemeRequestModel model)
        {
            ...
            _dbContext.MissionTheme.Add(missionTheme);
        ...
        {
            var query = _dbContext.MissionTheme.Select(m => new MissionThemeResponseModel(m)).AsQueryable();
        ...
        {
            var missionTheme = await _dbContext.MissionTheme.FindAsync(missionThemeId);
        ...
        {
            var missionTheme = _dbContext.MissionTheme.Find(model.Id);
            ...
            _dbContext.MissionTheme.Update(missionTheme);
            await _dbContext.SaveChangesAsync();
            return true;
        }
        ...
        {
            var missionTheme = _dbContext.MissionTheme.Find(id);

            if (missionTheme == null)
                return false;

            _dbContext.MissionTheme.Remove(missionTheme);
            await _dbContext.SaveChangesAsync();
            return true;
        }
    }
